...
G_DEFINE_TYPE(GCribbageApplicationWindow, gcribbage_application_window,
              GTK_TYPE_APPLICATION_WINDOW);

void new_game_clicked(GtkButton *button, GCribbageApplicationWindow *win) {
  gcribbage_table_start_new_game(GCRIBBAGE_TABLE(win->table));
}
...
static void gcribbage_application_window_class_init(
    GCribbageApplicationWindowClass *klass) {
  g_type_ensure(GCRIBBAGE_TABLE_TYPE);
  gtk_widget_class_set_template_from_resource(
      GTK_WIDGET_CLASS(klass), "/com/ronsbrain/gcribbage/window.ui");
  gtk_widget_class_bind_template_callback(GTK_WIDGET_CLASS(klass),
                                          new_game_clicked);
  gtk_widget_class_bind_template_child(GTK_WIDGET_CLASS(klass),
                                       GCribbageApplicationWindow, menu_button);
  gtk_widget_class_bind_template_child(GTK_WIDGET_CLASS(klass),
                                       GCribbageApplicationWindow, table);
}
...
gcribbage_application_window_new(GCribbageApplication *app) {
  return (GCribbageApplicationWindow *)g_object_new(
      GCRIBBAGE_APPLICATION_WINDOW_TYPE, "application", app, NULL);
}
